...
def _canonical_datetime_col(df: pd.DataFrame) -> str | None:
    for c in df.columns:
        if c.lower().strip() == "datetime":
            return c
    return None
...
def load_and_prepare(path: str) -> pd.DataFrame:
    ...
    dt_col = _canonical_datetime_col(df)
...
def select_safe_feature_columns(df: pd.DataFrame) -> list[str]:
    ...
    for col in df.columns:
        if dt_col is not None and col == dt_col:
            continue
        if col == DATETIME_COL:
            continue
        ...
        if col in SENSOR_COLS or col in METEO_COLS or col in ["Hour", "Weekday", "Month_sin", "Month_cos"]:
...
def make_xy(df: pd.DataFrame, target_col: str):
    safe_cols = select_safe_feature_columns(df)
    X = df[safe_cols].select_dtypes(include=[np.number]).copy()
    y = df[target_col].astype(float)

    non_numeric_in_safe = [c for c in safe_cols if df[c].dtype == "O"]
    if non_numeric_in_safe:
        raise ValueError(f"Discover non-numerical feature columns：{non_numeric_in_safe}")
...
test_fold = np.concatenate([
    -1 * np.ones(len(X_train), dtype=int),  # train
     0 * np.ones(len(X_valid), dtype=int)   # valid
])
...
valid_pred_best = pipe_tuned.predict(X_valid)
...
dt_col_test = _canonical_datetime_col(test_df) or DATETIME_COL
pred_df = pd.DataFrame({
    dt_col_test: test_df[dt_col_test].values if dt_col_test in test_df.columns else np.arange(len(y_test)),
    f"{TARGET}_actual": y_test.values,
    f"{TARGET}_pred": test_pred
})
pred_path = os.path.join(PRED_DIR, "pred_C6H6_exclS2.csv")
pred_df.to_csv(pred_path, index=False)
print(f"[{TARGET}] Saved predictions -> {pred_path}")

model_path = os.path.join(MODELS_DIR, "rf_C6H6_exclS2.joblib")
...
fi_path = os.path.join(OUT_DIR, "feature_importance_C6H6_exclS2.csv")
...
summary_path = os.path.join(OUT_DIR, "summary_C6H6_exclS2.csv")

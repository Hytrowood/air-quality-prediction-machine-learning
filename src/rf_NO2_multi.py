...
def make_xy_for_horizon(df: pd.DataFrame, target_col: str, h: int):
    ...
    feat_cols = select_safe_feature_columns(df_feat, target_col)
    ...
    dt = df.loc[mask, DATETIME_COL].reset_index(drop=True) if DATETIME_COL in df.columns else pd.Series(np.arange(len(y)))
...
print("\n" + "="*88)
print(f"Training multi-horizon (no leakage): {TARGET} @ {HORIZONS}")
print("="*88)
...
for h in HORIZONS:
    ...
    pred_path = os.path.join(PRED_DIR, f"pred_{safe_name}_h{h}.csv")
    pred_df.to_csv(pred_path, index=False)
    print(f"[h={h}] Saved predictions -> {pred_path}")

    model_path = os.path.join(MODELS_DIR, f"rf_{safe_name}_h{h}.joblib")
    ...
    fi_path = os.path.join(OUT_DIR, f"feature_importance_{safe_name}_h{h}.csv")
...
summary_path = os.path.join(OUT_DIR, "summary_metrics_CO_multi.csv")

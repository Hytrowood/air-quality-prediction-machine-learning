...
LAGS_SENSORS  = [1, 3, 6]          
SENSOR_LAG_SOURCES = ["PT08.S4(NO2)", "PT08.S5(O3)"]  
...
def load_and_prepare(path: str) -> pd.DataFrame:
    ...
    if ("Month_sin" not in df.columns) or ("Month_cos" not in df.columns):
        ...
        s, c = cyc_sin_cos(month, period=12, offset=1)
        df["Month_sin"] = s; df["Month_cos"] = c
...
def add_lags_inplace(df: pd.DataFrame):
    ...
    if TARGET not in df.columns:
        raise ValueError(f"The target column was not found {TARGET}")
    for k in LAGS_TARGET:
        df[f"{base}_lag{k}"] = df[TARGET].shift(k)

    for src in SENSOR_LAG_SOURCES:
        if src in df.columns:
            for k in LAGS_SENSORS:
                df[f"{src}_lag{k}"] = df[src].shift(k)

def trim_head_by_max_lag(df: pd.DataFrame) -> pd.DataFrame:
    maxlag = 0
    if LAGS_TARGET:
        maxlag = max(maxlag, max(LAGS_TARGET))
    if LAGS_SENSORS:
        maxlag = max(maxlag, max(LAGS_SENSORS))
    if maxlag <= 0:
        return df
    return df.iloc[maxlag:].reset_index(drop=True)
...
def select_safe_feature_columns(df: pd.DataFrame, target_col: str):
    ...
    for col in df.columns:
        ...
        if "(GT)" not in col:
            cols.append(col)
    leak_cols = [c for c in cols if "(GT)" in c]
    if leak_cols:
        raise ValueError(f"Detected leakage feature columns:{leak_cols}")
    return cols
...
def _rmse(y_true, y_pred):
    return math.sqrt(mean_squared_error(y_true, y_pred))
naive_rmse = _rmse(y_true_test_series.to_numpy().astype(float), y_naive.astype(float))
